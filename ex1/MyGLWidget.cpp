...
{
  puntMinEsc = glm::vec3(-2,-1,-2);
  puntMaxEsc = glm::vec3(2,3,2);  
  centreEsc = (puntMaxEsc + puntMinEsc)/ 2.0f;
  radiEsc = glm::distance(puntMaxEsc, centreEsc);  
  std::cout << "centreESC: " << centreEsc.x << ' ' << centreEsc.y << ' ' << centreEsc.z << " radiEsc: " 
  << radiEsc << std::endl; 
}
...
void MyGLWidget::viewTransform ()
{
  ...
  View = glm::translate(glm::mat4(1.f), glm::vec3(0, 0, -1.2*radiEsc));
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (DoingInteractive == ZOOM) {
    // Fem la zoom
    float var = (e->y() - yClick) * M_PI / 180.0; 
    if ((var + FOV > 0) and (var + FOV < M_PI))
      FOV += var;
    projectTransform ();
    emit enviaZoom(FOV/M_PI * 100);
  }
  xClick = e->x();
  yClick = e->y();
...
void MyGLWidget::actualitzaZoom(int newFov) {
    makeCurrent();
    FOV = float(newFov)/100 * M_PI;
    projectTransform();
    update();
